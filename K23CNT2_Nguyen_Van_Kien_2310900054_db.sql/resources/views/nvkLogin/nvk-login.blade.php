...
<body>
    ...
    <div class="wrap">
        ...
        <form action="{{ route('admins.nvkloginSubmit') }}" method="POST">
            ...
            <div class="user-box">
                <input type="text" name="nvkTaiKhoan" id="nvkTaiKhoan" value="{{ old('nvkTaiKhoan') }}" />
                <label for="nvkTaiKhoan">Username</label>
                @error('nvkTaiKhoan')
            ...
            <div class="user-box">
                <input type="password" name="nvkMatKhau" id="nvkMatKhau" />
                <label for="nvkMatKhau">Password</label>
                @error('nvkMatKhau')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
    ...
    <script>
      ...
      function toggleForgetPassword() {
        ...
        if (isChecked) {
          const username = document.getElementById('nvkTaiKhoan').value;
          const password = document.getElementById('nvkMatKhau').value;
          document.cookie = `nvkTaiKhoan=${username}; path=/; max-age=31536000`; // 1 year
          document.cookie = `nvkMatKhau=${password}; path=/; max-age=31536000`; // 1 year
        } else {
          // Remove cookies when "Remember me" is unchecked
          document.cookie = 'nvkTaiKhoan=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
          document.cookie = 'nvkMatKhau=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
        }
      }
      ...
      window.onload = function() {
        ...
        cookies.forEach(cookie => {
          if (cookie.trim().startsWith('nvkTaiKhoan=')) {
            username = cookie.trim().split('=')[1];
          }
          if (cookie.trim().startsWith('nvkMatKhau=')) {
            password = cookie.trim().split('=')[1];
          }
        });

        if (username && password) {
          document.getElementById('nvkTaiKhoan').value = username;
          document.getElementById('nvkMatKhau').value = password;
