        ...
        <div class="row">
            @foreach ($sanPhams as $sanPham)
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="{{ asset('storage/' . $sanPham->nvkHinhAnh) }}" class="card-img-top product-img" alt="{{ $sanPham->nvkTenSanPham }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $sanPham->nvkTenSanPham }}</h5>
                            <p class="card-text"><strong>{{ number_format($sanPham->nvkDonGia, 0, ',', '.') }} VND</strong></p>
                            <p class="card-text"><small class="text-muted">Số lượng còn lại: {{ $sanPham->nvkSoLuong }}</small></p>
                            
                            <!-- Nút xem chi tiết -->
                            <a href="{{ route('nvkuser.show', $sanPham->id) }}" class="btn btn-primary btn-sm">Xem Chi Tiết</a>

                            <!-- Nút Mua và icon giỏ hàng -->
                            <a href="{{route('nvkuser.login')}} "   
                            ...
                            <!-- Thêm vào giỏ hàng -->
                            <a href="{{route('nvkuser.login')}}"><button type="button" class="btn btn-warning btn-sm add-to-cart-btn" data-id="{{ $sanPham->id }}" data-name="{{ $sanPham->nvkTenSanPham }}">
