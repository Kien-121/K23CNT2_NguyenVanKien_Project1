...
class NVK_TIN_TUCController extends Controller
{
    ...
    {
    $nhttinTucs = NVK_TIN_TUC::all();
        
    // Phân trang kết quả, thay 10 bằng số lượng bạn muốn mỗi trang
    $nhttinTucs = NVK_TIN_TUC::paginate(10);
    
    
    // Return the view with the paginated data
    return view('nvkAdmins.nhttintuc.nht-list', ['nhttinTucs' => $nhttinTucs]);
}
    ...
    {
        $nhttinTuc = NVK_TIN_TUC::findOrFail($id);
        return view('nvkAdmins.nhttintuc.nht-detail', ['nhttinTuc' => $nhttinTuc]);
    }

    // Show the create form for a new news item ----------------------------------------
    public function nhtCreate()
    {
        return view('nvkAdmins.nhttintuc.nht-create');
    }
    ...
    {
        $nvktinTuc = NVK_TIN_TUC::findOrFail($id);
        return view('nvkAdmins.nvktintuc.nht-edit', ['nvktinTuc' => $nvktinTuc]);
    }
