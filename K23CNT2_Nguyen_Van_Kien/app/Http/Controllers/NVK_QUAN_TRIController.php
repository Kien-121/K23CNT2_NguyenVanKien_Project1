...
class NVK_QUAN_TRIController extends Controller
{
    ...
    {
        return view('nvklogin.nht-login');
    }
    ...
    {
        $nhtquantris = NVK_QUAN_TRI::all(); // Sử dụng Model NVK_QUAN_TRI
    ...
    {
        $nhtquantri = NVK_QUAN_TRI::where('id', $id)->first(); // Sử dụng Model NVK_QUAN_TRI
    ...
    public function nvkCreateSubmit(Request $request)
    {
        ...
        $nvkquantri->nvkTaiKhoan = $request->nhtTaiKhoan;
        $nvkquantri->nvkMatKhau = Hash::make($request->nhtMatKhau); // Mã hóa mật khẩu
        $nvkquantri->nvkTrangThai = $request->nhtTrangThai;
